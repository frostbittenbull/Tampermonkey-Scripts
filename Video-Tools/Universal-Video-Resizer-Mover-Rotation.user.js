...
// @description  Zero Auto-Resize, Render Bug Fix, Persistent Handles + Rotate Video Picture Only. Pure Hover Logic. Corner Resize. Zero-lag overlay. True Video Bounds. Prev/Next buttons + Picture-in-Picture. Liquid-glass handle. Smooth resize (no per-tick global resize storms).
// @author       frostbittenbull (modified)
// @icon         https://fonts.gstatic.com/s/i/short-term/release/materialsymbolsoutlined/picture_in_picture/default/48px.svg
// @icon64       https://fonts.gstatic.com/s/i/short-term/release/materialsymbolsoutlined/picture_in_picture/default/48px.svg
// @match        *://*/*
// @allFrames    true
// @grant        none
...
(function() {
    ...
    const GLASS_UNLOCKED = 'linear-gradient(135deg, rgba(226,43,43,0.35), rgba(226,43,43,0.18))';
    const GLASS_LOCKED = 'linear-gradient(135deg, rgba(43,200,110,0.35), rgba(43,200,110,0.18))';

    const moveHandle = document.createElement('div');
    moveHandle.style.cssText = `
        position: fixed; min-width: 85px; height: 26px; padding: 0 10px;
        cursor: pointer; z-index: ${Z_HANDLES};
        background: ${GLASS_UNLOCKED};
        -webkit-backdrop-filter: blur(14px) saturate(180%);
        backdrop-filter: blur(14px) saturate(180%);
        pointer-events: auto; display: none; transition: opacity 0.3s, background 0.3s;
        opacity: 0.45; border-radius: 13px; text-align: center;
        ...
        ${inIframe ? 'outline: 1.5px dashed rgba(255,235,59,0.85); outline-offset: 2px;' : ''}
    `;
    ...
    ['TL', 'TR', 'BL', 'BR'].forEach(corner => {
        ...
        handle.addEventListener('mousedown', (e) => {
            ...
            const _resizeStartPos = getCurrentLeftTop(activeVideo, getVideoBounds(activeVideo));
            startLeft = _resizeStartPos.left;
            startTop = _resizeStartPos.top;
    ...
    // Свойства на родителях, которые создают собственный "containing block" для
    // position:fixed (transform/filter/perspective/will-change/contain). Если у
    // предка видео такое свойство активно, наш fixed-видеоэлемент прижимается не
    // к viewport, а к этому предку — из-за этого при первом же drag/resize видео
    // "скачет" на разницу между настоящим viewport и рамкой предка (характерно для
    // YouTube Shorts, где обёртка плеера анимируется через transform).
    const ANCESTOR_PROPS_TO_NEUTRALIZE = ['transform', 'filter', 'perspective', 'will-change', 'contain', 'backdrop-filter'];

    // Некоторые плееры (похоже, в т.ч. Яндекс) реактивно перезаписывают inline-style
    // самого <video> в ответ на любое его изменение (например, привязывают размер
    // видео к какому-то внутреннему состоянию). Если просто один раз выставить стиль
    // через setStyles, сайт может почти сразу откатить его — снаружи это выглядит как
    // "скрипт не сработал". Поэтому храним последний применённый набор стилей и,
    // если style видео меняется не нами, — переигрываем его через MutationObserver.
    let lastVideoStyles = null;
    let uvrVideoSelfWrite = false;
    function setVideoStyles(styles) {
        if (!activeVideo) return;
        uvrVideoSelfWrite = true;
        setStyles(activeVideo, styles);
        lastVideoStyles = Object.assign({}, lastVideoStyles, styles);
        Promise.resolve().then(() => { uvrVideoSelfWrite = false; });
    }
    let videoStyleObserver = null;
    function ensureVideoStyleObserver() {
        if (videoStyleObserver) videoStyleObserver.disconnect();
        if (!activeVideo) return;
        videoStyleObserver = new MutationObserver(() => {
            if (uvrVideoSelfWrite || !isLocked || !activeVideo || !lastVideoStyles) return;
            console.debug('[UVR] сайт перезаписал style видео, переприменяю позицию/размер');
            setVideoStyles(lastVideoStyles);
        });
        videoStyleObserver.observe(activeVideo, { attributes: true, attributeFilter: ['style'] });
    }
    ...
    // Важно: полосы физически рисуются только когда object-fit — "contain"/"scale-down"
    // (или не задан явно — тогда браузер по умолчанию всё равно вписывает видео с сохранением
    // пропорций). Если сайт использует "cover" или "fill" (частый случай для вертикального
    // видео внутри горизонтального плеера — контент растягивают или обрезают, без полос),
    // коррекция по videoWidth/videoHeight только портит размеры и позицию — её нужно пропускать.
    function getVideoBounds(video) {
        ...
        let fitMode = 'contain';
        try { fitMode = getComputedStyle(video).objectFit || 'contain'; } catch (e) {}
        const isLetterboxed = (fitMode === 'contain' || fitMode === 'scale-down');

        if (isLetterboxed && video.videoWidth && video.videoHeight) {
    ...
    // Безопасно достаём left/top из inline style, не путая "0px" (валидное значение)
    // с "стиль не задан". Раньше `parseFloat(...) || bounds.left` подменял ровно 0
    // на bounds.left, из-за чего DRAG у видео, стоящего на x=0 или y=0 (частый случай
    // для полноэкранных вертикальных плееров типа YouTube Shorts), уводил его в сторону.
    function getCurrentLeftTop(el, bounds) {
        const rawLeft = el.style.left;
        const rawTop = el.style.top;
        const l = rawLeft ? parseFloat(rawLeft) : NaN;
        const t = rawTop ? parseFloat(rawTop) : NaN;
        return {
            left: Number.isNaN(l) ? bounds.left : l,
            top: Number.isNaN(t) ? bounds.top : t
        };
    }
    ...
    function updateHandlesPosition(precomputedBounds) {
        if (!activeVideo || !activeVideo.isConnected) return;
        const bounds = precomputedBounds || getVideoBounds(activeVideo);
        const size = 10;
    ...
    function hijackVideo() {
        ...
        // Поднимаем z-index предков И одновременно сбрасываем на них любые свойства,
        // которые создают собственный containing block для position:fixed (transform,
        // filter, perspective, will-change, contain). Иначе видео "прижимается" не к
        // реальному viewport, а к рамке такого предка — именно это вызывает скачок
        // видео при первом drag на YouTube Shorts, где обёртка плеера анимируется
        // через transform.
        let p = activeVideo.parentElement;
        while(p && p !== document.body && p !== document.documentElement) {
            if (!boostedParents.some(item => item.element === p)) {
                const saved = { element: p, originalZ: p.style.zIndex };
                ANCESTOR_PROPS_TO_NEUTRALIZE.forEach(prop => {
                    saved[prop] = { value: p.style.getPropertyValue(prop), priority: p.style.getPropertyPriority(prop) };
                    p.style.setProperty(prop, 'none', 'important');
                });
                p.style.setProperty('z-index', Z_PARENTS, 'important');
                boostedParents.push(saved);
            }
            p = p.parentElement;
        }
    ...
    function applyVideoTransform() {
        ...
        setVideoStyles({ 'transform': `rotate(${currentRotationInt}deg) scale(${scale})`, 'transform-origin': 'center center' });
    }
    ...
    function lockPlayer() {
        ...
        moveHandle.style.background = GLASS_LOCKED;
    ...
    function handleMouseMove(e) {
        ...
        if (isLocked) {
            if (!activeVideo || !activeVideo.isConnected) { unlockPlayer(); return; }
            updateHandlesPosition();
    ...
    function executeResize(e) {
        ...
        applyVideoTransform();
        // Один общий пересчёт bounds для хендлов и оверлея вместо двух отдельных —
        // каждый вызов getBoundingClientRect() после записи стилей форсирует
        // синхронный reflow, так что два вызова подряд означают двойную стоимость
        // на каждый кадр перетаскивания.
        const _bounds = activeVideo ? getVideoBounds(activeVideo) : null;
        if (_bounds) {
            updateHandlesPosition(_bounds);
            if (syncOverlayPosition) syncOverlayPosition(_bounds);
        }
        // dispatchEvent('resize') специально НЕ вызываем на каждый тик: это будит
        // все resize-обработчики на странице (включая тяжёлые сторонние — плеер,
        // рекламу, чужие layout-скрипты), и именно это давало ощутимый лаг при
        // перетаскивании. Одного дисперча в конце жеста (см. stopDrag) достаточно,
        // чтобы сайт подстроил свой внутренний UI под финальный размер.
    }
    ...
    moveHandle.addEventListener('mousedown', (e) => {
        ...
        const bounds = getVideoBounds(activeVideo);
        const cur = getCurrentLeftTop(activeVideo, bounds);

        moveOffsetX = e.clientX - cur.left;
        moveOffsetY = e.clientY - cur.top;
        e.preventDefault();
    });
    ...
    function unlockPlayer() {
        ...
        const isStillInDOM = activeVideo.isConnected;

        if (videoStyleObserver) { videoStyleObserver.disconnect(); videoStyleObserver = null; }
        lastVideoStyles = null;
        ...
        boostedParents.forEach(item => {
            if (item.element) {
                ...
                ANCESTOR_PROPS_TO_NEUTRALIZE.forEach(prop => {
                    const saved = item[prop];
                    if (!saved) return;
                    if (saved.value) item.element.style.setProperty(prop, saved.value, saved.priority);
                    else item.element.style.removeProperty(prop);
                });
            }
        });
        boostedParents = [];
        ...
        moveHandle.style.background = GLASS_UNLOCKED;
        moveHandle.innerText = TEXT_DRAG;
    ...
    _pt.addEventListener('mouseleave',()=>{_pt.style.height='4px';if(!_isSeeking)_pd.style.opacity='0';});
    ...
    function _syncPos(precomputedBounds){
        if(!activeVideo)return;
        const b = precomputedBounds || getVideoBounds(activeVideo);
    ...
    function _startSeek(e){
        e.stopPropagation();e.preventDefault();
        if(!activeVideo||!activeVideo.duration)return;
        _isSeeking=true;
        _pd.style.opacity='1';
        _seekTo(e.clientX);
        const onMove=(ev)=>_seekTo(ev.clientX);
        const onUp=()=>{
            _isSeeking=false;
            _pd.style.opacity='0';
    ...
    _pt.addEventListener('mousedown',_startSeek);
    _pd.addEventListener('mousedown',_startSeek);
    function _volWheel(e){
        e.stopPropagation();e.preventDefault();
        if(!activeVideo)return;
        const cur = activeVideo.muted ? 0 : activeVideo.volume;
        const delta = e.deltaY < 0 ? 0.05 : -0.05;
        const v = Math.max(0, Math.min(1, Math.round((cur + delta) * 100) / 100));
        activeVideo.volume = v;
        activeVideo.muted = v === 0;
        _syncVol();
    }
    _vt.addEventListener('wheel', _volWheel, {passive:false});
    _vbtn.el.addEventListener('wheel', _volWheel, {passive:false});
